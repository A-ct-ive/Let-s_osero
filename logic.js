...
function rev(x, y, add_x, add_y) {
  ...
  while (true) {
    ...
    //ヒント1 ある条件の時だけflagに1を持たせる -> コマがx,yに置けるか判定をここでしている
    //ヒント2 6パターンあります! うまく書けば3パターンになります．
    //----------------------------------------------
    // if(~~~~){
    //   break
    // }
...
function skip() {
  //バックアップを取らないとcheck()によって中身が変わってしまう
  ...
  let count_can = 0;
  //問題 5 自分のコマが置けない場合は相手のターンに変更
  //問題 5-1 全部のマスを自分のコマがおけるかおけないかチェック
  //ヒント check関数の再利用.全部のマスを見る為にはどうする？
  //--------------------------------------
